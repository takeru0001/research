...
passenger_distribution = [
    [0 for i in range(num_to_divide_x)] for j in range(num_to_divide_y)
]
...
for taxi_id in id_list_of_taxis:
    ...
    for lat, lng, ride_state, unixtime in sorted_data_list:
        if is_first_line:
            prev_lat, prev_lng, prev_ride_state, prev_unixtime = (
                lat,
                lng,
                ride_state,
                unixtime,
        ...
        if ride_state == 0 and prev_ride_state == 1:
            ...
            if (
                0 <= x_coordinate < num_to_divide_x
                and 0 <= y_coordinate < num_to_divide_y
            ):
