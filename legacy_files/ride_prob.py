        ...
        for data_dict in data_lists:
            ...
            if not (prev_ride_state == 0 and ride_state == 0):
                ...
                if prev_ride_state == 0 and ride_state == 1:
                    ...
                    if (
                        reward is not None and 2400 > elapsed_time > 120 and reward < 40
...
def find_ride_num_reward_each_area(
    num_of_division, origBoundary, ride_points, reward_list
):
    top = origBoundary[3]
    ...
    reward_each_area = [
        [[] for i in range(num_of_division)] for j in range(num_of_division)
    ]
...
def find_ride_prob(num_of_division, ride_num_each_area):
    ...
    for k in range(24):
        ...
        for i in range(num_of_division):
            for j in range(num_of_division):
                ride_prob[k][i][j] = (
                    ride_num_each_area[k][i][j] * prob_increase_per_a_ride
                )
...
def get_ride_prob_and_reward(filename_of_xml, num_of_division):
    ...
    ride_num_each_area, reward_each_area = find_ride_num_reward_each_area(
        num_of_division, origBoundary, ride_points, reward_list
    )
