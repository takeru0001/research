...
class Car:
    ...
    def init(self, DG):
        ...
        current_edge_attributes = DG.get_edge_data(
            current_start_node_id, current_end_node_id
        )
    ...
    def V(self, inter_car_distance):
        return (
            ...
            * (np.tanh(inter_car_distance - 2) + np.tanh(2))
        )

    # update car's speed
    def update_current_speed(self, sensitivity, inter_car_distance):
        self.current_speed += sensitivity * (
            self.V(inter_car_distance) - self.current_speed
        )
    ...
    def move(self, DG, edges_cars_dic, sensitivity):
        ...
        if (
            np.sqrt(
                (self.current_position[0] - self.current_end_node[0]) ** 2
                + (self.current_position[1] - self.current_end_node[1]) ** 2
            )
            < self.current_speed
        ):  # to arrive at the terminal of edge
            self.current_sp_index += 1

            if (
                self.current_sp_index >= len(self.shortest_path) - 1
            ):  # arrived at the goal
                ...
                edges_cars_dic[(current_start_node_id, current_end_node_id)].remove(
                    self
                )
                ...
                self.current_max_speed = current_edge_attributes["speed"]
                self.current_distance = current_edge_attributes["weight"]
                edges_cars_dic[(current_start_node_id, current_end_node_id)].append(
                    self
                )
            ...
            if (
                edges_cars_dic[(current_start_node_id, current_end_node_id)].index(self)
                > 0
            ):
                car_forward_index = (
                    edges_cars_dic[(current_start_node_id, current_end_node_id)].index(
                        self
                    )
                    - 1
                )
                car_forward_pt = edges_cars_dic[
                    (current_start_node_id, current_end_node_id)
                ][car_forward_index]
                diff_dist = np.sqrt(
                    (car_forward_pt.current_position[0] - self.current_position[0]) ** 2
                    + (car_forward_pt.current_position[1] - self.current_position[1])
                    ** 2
                )
            else:
                diff_dist = 50.0
