    ...
    try:
        ...
        for line in infile:
            ...
            if prev_ride_state == 1 and ride_state == 0:
                trace_list.append(
                    (
                        round(latitude, effective_digits),
                        round(longitude, effective_digits),
                    )
                )
        ...
        R_g2_cm_lon = 0
        R_g2_cm_lat = (
            sorted_frequency_dict[0][1] * sorted_frequency_dict[0][0][0]
            + sorted_frequency_dict[1][1] * sorted_frequency_dict[1][0][0]
        ) / (sorted_frequency_dict[0][1] + sorted_frequency_dict[1][1])
        R_g2_cm_lon = (
            sorted_frequency_dict[0][1] * sorted_frequency_dict[0][0][1]
            + sorted_frequency_dict[1][1] * sorted_frequency_dict[1][0][1]
        ) / (sorted_frequency_dict[0][1] + sorted_frequency_dict[1][1])

        # r_g^{(2)}の計算
        R_g2 = 0
        R_g2 = sorted_frequency_dict[0][1] * (
            (sorted_frequency_dict[0][0][0] - R_g2_cm_lat) ** 2
            + (sorted_frequency_dict[0][0][1] - R_g2_cm_lon) ** 2
        ) + sorted_frequency_dict[1][1] * (
            (sorted_frequency_dict[1][0][0] - R_g2_cm_lat) ** 2
            + (sorted_frequency_dict[1][0][1] - R_g2_cm_lon) ** 2
        )
        R_g2 = np.sqrt(
            R_g2 / float(sorted_frequency_dict[0][1] + sorted_frequency_dict[1][1])
        )
        ...
        if s2 > 0.9 and (
            sorted_frequency_dict[0][0][0] == 37.62
            or sorted_frequency_dict[1][0][0] == 37.62
        ):
