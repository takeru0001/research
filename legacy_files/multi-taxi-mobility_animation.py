...
# smopy setting
pos0 = (37.45708, -122.5197584)
pos1 = (37.95071, -121.97057)
...
if not os.path.exists("SAN-basemap-z%d.png" % (z)):
    ...
    json.dump(
        {
            ...
            "box3": mapobj.box[3],
            "z": z,
            "box_tile0": mapobj.box_tile[0],
            "box_tile1": mapobj.box_tile[1],
            "box_tile2": mapobj.box_tile[2],
            "box_tile3": mapobj.box_tile[3],
            "xmin": mapobj.xmin,
            "ymin": mapobj.ymin,
        },
        outfile,
    ...
    # load json
    infile = open("basemap-z%d.json" % (z))
    ...
    z = int(json_dic["z"])
    box_tile = (
        float(json_dic["box_tile0"]),
        float(json_dic["box_tile1"]),
        float(json_dic["box_tile2"]),
        float(json_dic["box_tile3"]),
    )
...
for i in range(len(all_data_sorted_list)):
    ...
    if unixtime_now >= unixtime_old + minute_interval * 60:  # every 1 min.
        ...
        ax.imshow(img)
        timestamp_str = (
            str(dt.datetime.fromtimestamp(unixtime_old, PST))[:-6] + " " + str(PST)
        )
        timestamp_str2 = (
            str(dt.datetime.fromtimestamp(unixtime_old + minute_interval * 60, PST))[
                :-6
            ]
            + " "
            + str(PST)
        ...
        for j in range(len(x_to_pixcel)):
            plt.plot(
                ...
                markersize=3,
                linestyle="",
                color=colors[label_list[j] % len(colors)],
            )
    ...
    if get_mapobj == False:
        x, y = to_pixels(
            float(all_data_sorted_list[i][3]),
            float(all_data_sorted_list[i][2]),
            z,
            box_tile,
        )
        x_to_pixcel.append(x)
        y_to_pixcel.append(y)
        label_list.append(all_data_sorted_list[i][1])
    else:
        x, y = mapobj.to_pixels(
            float(all_data_sorted_list[i][3]), float(all_data_sorted_list[i][2])
        )
        x_to_pixcel.append(x)
        y_to_pixcel.append(y)
        label_list.append(all_data_sorted_list[i][1])
