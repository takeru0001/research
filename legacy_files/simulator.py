...
def get_map_smopy():
    ...
    px_min_lon, px_min_lat = smopy_map.to_pixels(
        lat=lon_lat_tuple[0], lon=lon_lat_tuple[1]
    )
    px_max_lon, px_max_lat = smopy_map.to_pixels(
        lat=lon_lat_tuple[2], lon=lon_lat_tuple[3]
    )
...
def create_road_network(root, smopy_map):
    ...
    def calculate_coordinates(convBoundary, origBoundary, node_x, node_y):
        orig_per_conv_X = abs(origBoundary[0] - origBoundary[2]) / abs(
            convBoundary[0] - convBoundary[2]
        )
        orig_per_conv_Y = abs(origBoundary[1] - origBoundary[3]) / abs(
            convBoundary[1] - convBoundary[3]
        )
            ...
            for child2 in child:
                ...
                for data in data_list:
                    ...
                    if data_counter >= 1:
                        distance_list.append(
                            np.sqrt(
                                (node_x - old_node_x) ** 2 + (node_y - old_node_y) ** 2
                            )
                        )
                    old_node_x = node_x
                    old_node_y = node_y
                ...
                for i in range(len(node_id_list) - 1):
                    DG.add_edge(
                        ...
                        color="black",
                        speed=float(child2.attrib["speed"]),
                    )  # calculate weight here
                if "from" in child.attrib and "to" in child.attrib:
                    lane.set_others(
                        float(child2.attrib["speed"]),
                        node_id_list,
                        node_x_list,
                        node_y_list,
                    )
        ...
        if largest_scc == True:
            ...
            for i in range(len(c_list)):
                ...
                for lane in edge_lanes_list:
                    if lane.node_id_list[0] == int(c_list[i]) or lane.node_id_list[
                        1
                    ] == int(c_list[i]):
...
def create_road_segments(edge_lanes_list):
    ...
    for i in range(len(edge_lanes_list) - 1):
        for j in range(i + 1, len(edge_lanes_list)):
            if (
                edge_lanes_list[i].from_id == edge_lanes_list[j].to_id
                and edge_lanes_list[i].to_id == edge_lanes_list[j].from_id
            ):
                road_segments_list.append(
                    RoadSegment(edge_lanes_list[i], edge_lanes_list[j])
                )
                break
...
def find_OD_node_ids(origin_lane_id, destination_lane_id, x_y_dic):
    origin_node_id = x_y_dic[
        (
            edge_lanes_list[origin_lane_id].node_x_list[0],
            edge_lanes_list[origin_lane_id].node_y_list[0],
        )
    ]
    destination_node_id = x_y_dic[
        (
            edge_lanes_list[destination_lane_id].node_x_list[-1],
            edge_lanes_list[destination_lane_id].node_y_list[-1],
        )
    ]
...
def animate(time):
    ...
    def choose_dest_node_at_random():
        ...
        dest_node_id = x_y_dic[
            (
                edge_lanes_list[dest_lane_id].node_x_list[-1],
                edge_lanes_list[dest_lane_id].node_y_list[-1],
            )
        ]
    ...
    for car in cars_list:
        ...
        if car.goal_arrived == True:
            ...
            if (orig_index_x, orig_index_y) in car.experience[index_time].keys():
                car.experience[index_time][(orig_index_x, orig_index_y)][
                    "step"
                ] += car.num_of_elapsed_steps

                step = car.experience[index_time][(orig_index_x, orig_index_y)]["step"]
                reward = car.experience[index_time][(orig_index_x, orig_index_y)][
                    "reward"
                ]
                car.experience[index_time][(orig_index_x, orig_index_y)][
                    "reward per step"
            ...
            # 目的地の設定
            while True:
                if ride_prob[index_time][index_y][index_x] >= random.random() and len(
                    reward_each_area[index_y][index_x]
                ):
                    # そのエリアで乗客を拾える場合
                    ride_flag = True

                    passenger_num_in_the_area = random.randrange(
                        len(reward_each_area[index_y][index_x])
                    )
                    dest_x = reward_each_area[index_y][index_x][
                        passenger_num_in_the_area
                    ]["index_x"]
                    dest_y = reward_each_area[index_y][index_x][
                        passenger_num_in_the_area
                    ]["index_y"]
                    ...
                    try:
                        ...
                        reward_each_area[index_y][index_x].pop(
                            passenger_num_in_the_area
                        )
                        continue
                    ...
                    # 現在地からの距離を考慮した、過去の経験からの目的地の設定
                        ...
                        if len(car.experience[index_time]) == 0:  # 過去の経験がない場合はランダムで選ぶ
                            ...
                            for index, experience in car.experience[index_time].items():
                                ...
                                distance = np.sqrt(
                                    (x_diff * X) ** 2 + (y_diff * Y) ** 2
                                )  # 現在地から仮の目的地への距離
                                # print("distance", distance)
                                tmp_reward = -(distance / 10)  # 仮の目的地への移動コスト
                                if experience["count"] != 0:
                                    step_per_count = (
                                        experience["step"] / experience["count"]
                                    )
                                ...
                                tmp_reward_per_step = (
                                    experience["reward per step"] + tmp_reward
                                )
                ...
                if ride_flag:
                    for i in car.experience[index_time]:
                        if (index_x, index_y) in i.keys():
                            i[(index_x, index_y)]["reward"] += reward_each_area[
                                index_y
                            ][index_x][passenger_num_in_the_area]["reward"]
                            break
                    car.total_reward += reward_each_area[index_y][index_x][
                        passenger_num_in_the_area
                    ]["reward"]
                    # ride_flag = False
                    # print("get reward")
            ...
            # 移動によるマイナス報酬の設定
            orig_pos = (
                node_id_to_coordinate[orig_node_id]["latitude"],
                node_id_to_coordinate[orig_node_id]["longitude"],
            )
            dest_pos = (
                node_id_to_coordinate[dest_node_id]["latitude"],
                node_id_to_coordinate[dest_node_id]["longitude"],
            )
            ...
            edges_cars_dic[
                (
                    new_car.shortest_path[new_car.current_sp_index],
                    new_car.shortest_path[new_car.current_sp_index + 1],
                )
            ].append(new_car)
    ...
    title.set_text(
        "Simulation step: " + str(time) + ";  # of cars: " + str(len(cars_list))
    )
...
if __name__ == "__main__":
    ...
    X = (
        dist_on_sphere(
            (lon_lat_tuple[0], lon_lat_tuple[1]), (lon_lat_tuple[0], lon_lat_tuple[3])
        )
        / num_of_division
    )
    Y = (
        dist_on_sphere(
            (lon_lat_tuple[0], lon_lat_tuple[1]), (lon_lat_tuple[2], lon_lat_tuple[1])
        )
        / num_of_division
    )
    ...
    (
        ...
        index_to_node_id,
        node_id_to_coordinate,
    ) = create_road_network(root, smopy_map)
    ...
    while i < number_of_cars:
        ...
        origin_node_id, destination_node_id = find_OD_node_ids(
            origin_lane_id, destination_lane_id, x_y_dic
        )
        ...
        edges_cars_dic[
            (
                edge_lanes_list[origin_lane_id].node_id_list[0],
                edge_lanes_list[origin_lane_id].node_id_list[1],
            )
        ].append(car)
        i += 1
    ...
    (dest,) = plt.plot(
        [], [], color="red", marker="*", linestyle="", markersize=6
    )  # 目的地のプロット
    (dest_ride,) = plt.plot(
        [], [], color="blue", marker="*", linestyle="", markersize=6
    )  # 目的地のプロット
    ...
    ani = FuncAnimation(
        fig, animate, frames=range(100), init_func=init, blit=True, interval=50
    )
    ...
    with open(
        os.path.join(save_dir, "total_reward_" + str(epsilon) + ".txt"), "w"
    ) as f:
        for reward in total_rewards:
            f.write(str(reward) + "\n")

    with open(
        os.path.join(save_dir, "destination_coordinates_data" + str(epsilon) + ".txt"),
        "w",
    ) as f:
        for car_id_data, time_data, orig_pos_data, dest_pos_data in zip(
            car_id_datas, time_datas, orig_pos_datas, dest_pos_datas, strict=False
        ):
            f.write(
                str(car_id_data)
                + ","
